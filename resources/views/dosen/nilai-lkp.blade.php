...
<head>
    ...
    <title>Nilai Seminar LKP - Dosen</title>
...
<aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-menu">
        ...
        <a href="{{ route('dosen.nilai-lkp.index') }}" class="menu-item active"><i class="fas fa-star-half-alt"></i>Nilai Seminar LKP</a>
...
<div class="main">
    <nav class="topbar">
        <div style="display:flex;align-items:center;gap:1rem;"><button class="toggle-btn" id="sidebarToggle"><i class="fas fa-bars"></i></button><span style="font-weight:600;font-size:.95rem;">Nilai Seminar LKP</span></div>
    ...
    <div class="content">
        ...
        <div class="table-card">
            <div class="table-header">
                <div>
                    <strong style="font-size:1rem;">Input &amp; Nilai Seminar LKP</strong>
                    <p style="color:var(--muted);font-size:.83rem;margin:0;">Mahasiswa yang Anda bimbing/uji &middot; Total: {{ $seminarList->count() }}</p>
                </div>
            ...
            <div class="table-responsive">
                <table class="custom-table" id="nilaiTable">
                    <thead>
                        <tr>
                            <th rowspan="2">No</th>
                            <th rowspan="2">Judul LKP</th>
                            <th rowspan="2">Nama</th>
                            <th rowspan="2">NIM</th>
                            <th rowspan="2">Peran Saya</th>
                    ...
                    <tbody>
                        ...
                        @php
                            $nilai = $s->nilai;
                            $formId = 'form-nilai-' . $s->id;
                            $peran = $s->pembimbing1_id == $dosen->id ? 'Pembimbing' : 'Penguji';
                        @endphp
                        <tr>
                            <td>{{ $i+1 }}</td>
                            <td class="text-start">{{ Str::limit($s->judul_lkp, 30) }}</td>
                            <td class="text-start"><strong>{{ $s->mahasiswa->nama ?? '-' }}</strong></td>
                        ...
                        @empty
                        <tr><td colspan="11"><div class="empty-state"><i class="fas fa-inbox"></i><p>Belum ada seminar LKP yang Anda bimbing/uji.</p></div></td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        ...
        @foreach($seminarList as $s)
            <form id="form-nilai-{{ $s->id }}" action="{{ route('dosen.nilai-lkp.store', $s->id) }}" method="POST" style="display:none;">@csrf</form>
        @endforeach
    </div>
</div>
