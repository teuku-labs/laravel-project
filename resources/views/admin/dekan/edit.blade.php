...
<div style="margin-bottom:2rem;">
    <a href="{{ route('admin.dekan.index') }}" style="color:var(--primary-green);text-decoration:none;font-size:.9rem;"><i class="fas fa-arrow-left"></i> Kembali</a>
    <h2 style="font-weight:700;margin-top:.5rem;">Edit Dekan</h2>
</div>
@if($errors->any())
    ...
    <form action="{{ route('admin.dekan.update', $dekan->id) }}" method="POST">
        ...
        <div style="display:flex;flex-direction:column;gap:1.25rem;">
            <div>
                ...
                <input type="text" name="nama" value="{{ old('nama', $dekan->nama) }}" class="form-control" required>
            </div>
            <div>
                <label style="font-size:.85rem;font-weight:500;margin-bottom:.4rem;display:block;">NUPTK <span style="color:red">*</span></label>
                <input type="text" name="nuptk" value="{{ old('nuptk', $dekan->nuptk) }}" class="form-control" required>
            </div>
            <div>
                ...
                <select name="prodi_id" class="form-select" required>
                    ...
                    @foreach($prodis as $p)
                    <option value="{{ $p->id }}" {{ old('prodi_id', $dekan->prodi_id) == $p->id ? 'selected' : '' }}>{{ $p->nama }}</option>
                    @endforeach
                </select>
            </div>
