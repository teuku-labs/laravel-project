...
<head>
    ...
    <title>Nilai Seminar LKP - Dekan</title>
...
<div class="main">
    <nav class="topbar">
        <div style="display:flex;align-items:center;gap:1rem;">
            ...
            <span style="color:var(--muted);font-size:.9rem;">Nilai Seminar LKP</span>
        </div>
        <div style="display:flex;align-items:center;gap:.75rem;">
            <div class="avatar">{{ substr($dekan->nama ?? 'D', 0, 1) }}</div>
            <div><div class="uname">{{ $dekan->nama ?? '-' }}</div><div class="urole">Dekan</div></div>
        </div>
    </nav>
    ...
    <div class="content">
            ...
            <div class="table-header">
                <div>
                    ...
                    <p style="color:var(--muted);font-size:.83rem;margin:0;">
                        Total: {{ $seminarList->count() }} mahasiswa &middot;
                        Anda hanya bisa mengisi nilai untuk seminar yang Anda menjadi pembimbing/penguji.
                    </p>
                </div>
            ...
            <div class="table-responsive">
                <table class="custom-table" id="nilaiTable">
                    ...
                    <tbody>
                        ...
                        @php
                            $nilai = $s->nilai;
                            $berhak = $dosenSaya && ($s->pembimbing1_id == $dosenSaya->id || $s->penguji_id == $dosenSaya->id);
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @else
                                    <span class="badge-locked"><i class="fas fa-lock"></i> Bukan penguji/pembimbing Anda</span>
                                @endif
                            </td>
                        </tr>
                        @empty
                        <tr><td colspan="10"><div class="empty-state"><i class="fas fa-inbox"></i><p>Belum ada data seminar LKP.</p></div></td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        ...
        @foreach($seminarList as $s)
            <form id="form-nilai-{{ $s->id }}" action="{{ route('dekan.nilai-lkp.store', $s->id) }}" method="POST" style="display:none;">
                @csrf
            </form>
        @endforeach
    </div>
