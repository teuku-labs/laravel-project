...
<head>
    ...
    <title>Nilai Seminar Proposal - Dekan</title>
...
<div class="main">
    <nav class="topbar">
        <div style="display:flex;align-items:center;gap:1rem;">
            ...
            <span style="color:var(--muted);font-size:.9rem;">Nilai Seminar Proposal</span>
        </div>
    ...
    <div class="content">
            ...
            <div class="table-header">
                <div>
                    ...
                    <p style="color:var(--muted);font-size:.83rem;margin:0;">
                        Total: {{ $proposalList->count() }} mahasiswa &middot;
                        Anda hanya bisa mengisi nilai untuk proposal yang Anda menjadi pembimbing.
                    </p>
                </div>
            ...
            <div class="table-responsive">
                <table class="custom-table" id="nilaiTable">
                    ...
                    <tbody>
                        ...
                        @php
                            $nilai = $p->nilai;
                            $berhak = $dosenSaya && ($p->pembimbing1_id == $dosenSaya->id);
                            $formId = 'form-nilai-' . $p->id;
                        @endphp
                        <tr>
                            <td>{{ $i+1 }}</td>
                            <td class="text-start">{{ Str::limit($p->judul, 30) }}</td>
                            <td class="text-start"><strong>{{ $p->mahasiswa->nama ?? '-' }}</strong></td>
                        ...
                        @empty
                        <tr><td colspan="10"><div class="empty-state"><i class="fas fa-inbox"></i><p>Belum ada data seminar proposal.</p></div></td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        ...
        @foreach($proposalList as $p)
            <form id="form-nilai-{{ $p->id }}" action="{{ route('dekan.nilai-proposal.store', $p->id) }}" method="POST" style="display:none;">
                @csrf
            </form>
        @endforeach
    </div>
