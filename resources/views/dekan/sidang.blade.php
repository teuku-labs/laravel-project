...
<head>
    ...
    <title>Sidang Skripsi - Dekan</title>
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box;}body{font-family:'Poppins',sans-serif;background:var(--bg);color:var(--text);}
        ...
        .stat-value{font-size:1.6rem;font-weight:700;line-height:1;}.stat-label{font-size:.78rem;color:var(--muted);}
        ...
        .badge-approved{background:rgba(52,152,219,.15);color:#2980b9;padding:.3rem .8rem;border-radius:20px;font-size:.73rem;font-weight:500;}
        .btn-file{color:var(--primary);font-size:.83rem;text-decoration:none;display:inline-flex;align-items:center;gap:.3rem;}
...
<aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-menu">
        ...
        <a href="{{ route('dekan.proposal') }}" class="menu-item"><i class="fas fa-file-contract"></i>Proposal Skripsi</a>
        <a href="{{ route('dekan.sidang') }}" class="menu-item active"><i class="fas fa-gavel"></i>Sidang Skripsi</a>
...
<div class="main">
    <nav class="topbar">
        <div style="display:flex;align-items:center;gap:1rem;"><button class="toggle-btn" id="sidebarToggle"><i class="fas fa-bars"></i></button><span style="color:var(--muted);font-size:.9rem;">Sidang Skripsi</span></div>
    ...
    <div class="content">
        <div style="margin-bottom:1.5rem;"><h2 style="font-weight:700;">Sidang Skripsi</h2><p style="color:var(--muted);font-size:.9rem;">Daftar semua pengajuan sidang skripsi mahasiswa</p></div>

        <div class="stats-grid">
            <div class="stat-card"><div class="stat-icon" style="background:rgba(225,112,85,.12);color:var(--primary)"><i class="fas fa-gavel"></i></div><div><div class="stat-value">{{ $sidangList->count() }}</div><div class="stat-label">Total Sidang</div></div></div>
            <div class="stat-card"><div class="stat-icon" style="background:rgba(243,156,18,.12);color:#f39c12"><i class="fas fa-clock"></i></div><div><div class="stat-value">{{ $sidangList->where('admin_verified',false)->count() }}</div><div class="stat-label">Belum Verifikasi</div></div></div>
            <div class="stat-card"><div class="stat-icon" style="background:rgba(52,152,219,.12);color:#3498db"><i class="fas fa-check"></i></div><div><div class="stat-value">{{ $sidangList->where('admin_verified',true)->count() }}</div><div class="stat-label">Terverifikasi</div></div></div>
            <div class="stat-card"><div class="stat-icon" style="background:rgba(39,174,96,.12);color:#27ae60"><i class="fas fa-check-double"></i></div><div><div class="stat-value">{{ $sidangList->where('kaprodi_approved',true)->count() }}</div><div class="stat-label">Disetujui Kaprodi</div></div></div>
        </div>

        <div class="table-card">
            <div class="table-header">
                <strong>Semua Pengajuan Sidang</strong>
                <div class="search-box"><i class="fas fa-search"></i><input type="text" id="searchInput" placeholder="Cari mahasiswa..."></div>
            </div>
            <div class="table-responsive">
                <table class="custom-table" id="sidangTable">
                    <thead><tr><th>No</th><th>Mahasiswa</th><th>NIM</th><th>Prodi</th><th>Judul Skripsi</th><th>Pembimbing</th><th>Berkas</th><th>Status</th></tr></thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style="display:flex;gap:.5rem;flex-wrap:wrap;">
                                <a href="{{ asset('storage/'.$s->file_draft) }}" target="_blank" class="btn-file"><i class="fas fa-file-pdf"></i> Skripsi</a>
                                @if($s->file_bebas_pustaka)
                                <a href="{{ asset('storage/'.$s->file_bebas_pustaka) }}" target="_blank" class="btn-file"><i class="fas fa-file"></i> Bebas Pustaka</a>
                                @endif
                            </td>
                            <td>
                                @if($s->kaprodi_approved)
                                    <span class="badge-approved"><i class="fas fa-check-double"></i> Disetujui</span>
                                @elseif($s->admin_verified)
                                    <span class="badge-done"><i class="fas fa-check"></i> Terverifikasi</span>
                                @else
                        ...
                        @empty
                        <tr><td colspan="8"><div class="empty-state"><i class="fas fa-gavel"></i><p>Belum ada pengajuan sidang skripsi.</p></div></td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
...
<script>
    ...
    document.getElementById('searchInput').addEventListener('input',function(){const q=this.value.toLowerCase();document.querySelectorAll('#sidangTable tbody tr').forEach(r=>{r.style.display=r.textContent.toLowerCase().includes(q)?'':'none';});});
</script>
</body>
</html>