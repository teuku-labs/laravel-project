...
<head>
    ...
    <title>Sidang Skripsi - Kaprodi</title>
...
<aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-menu">
        ...
        <a href="{{ route('kaprodi.sk-pembimbing.index') }}" class="menu-item"><i class="fas fa-file-signature"></i>SK Pembimbing</a>
        <a href="{{ route('kaprodi.sidang.index') }}" class="menu-item active"><i class="fas fa-gavel"></i>Sidang Skripsi
            <span class="badge-menu">{{ $sidangs->where('kaprodi_approved',false)->where('kaprodi_rejected',false)->count() }}</span>
        </a>
...
<div class="main">
    ...
    <div class="content">
        ...
        <div style="margin-bottom:2rem;"><h2 style="font-weight:700;">Sidang Skripsi</h2><p style="color:var(--muted);font-size:.9rem;">Pengajuan sidang yang sudah diverifikasi admin dan menunggu keputusan Kaprodi</p></div>

        <div class="stats-grid">
            <div class="stat-card"><div class="stat-icon" style="background:rgba(243,156,18,.12);color:#f39c12"><i class="fas fa-clock"></i></div><div><div class="stat-value">{{ $sidangs->where('kaprodi_approved',false)->where('kaprodi_rejected',false)->count() }}</div><div class="stat-label">Menunggu</div></div></div>
            <div class="stat-card"><div class="stat-icon" style="background:rgba(39,174,96,.12);color:#27ae60"><i class="fas fa-check-circle"></i></div><div><div class="stat-value">{{ $sidangs->where('kaprodi_approved',true)->count() }}</div><div class="stat-label">Disetujui</div></div></div>
            <div class="stat-card"><div class="stat-icon" style="background:rgba(231,76,60,.12);color:#e74c3c"><i class="fas fa-times-circle"></i></div><div><div class="stat-value">{{ $sidangs->where('kaprodi_rejected',true)->count() }}</div><div class="stat-label">Ditolak</div></div></div>
            <div class="stat-card"><div class="stat-icon" style="background:rgba(108,92,231,.12);color:var(--primary)"><i class="fas fa-list"></i></div><div><div class="stat-value">{{ $sidangs->total() }}</div><div class="stat-label">Total</div></div></div>
        </div>

        <div class="table-card">
            <div class="table-header">
                <strong>Daftar Pengajuan Sidang</strong>
                <span style="font-size:.83rem;color:var(--muted);">{{ $sidangs->total() }} pengajuan</span>
            </div>
            <div class="table-responsive">
                <table class="custom-table">
                    <thead><tr><th>No</th><th>Mahasiswa</th><th>NIM</th><th>Judul Skripsi</th><th>Pembimbing</th><th>Status</th><th>Aksi</th></tr></thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><a href="{{ route('kaprodi.sidang.show', $s->id) }}" class="btn-detail"><i class="fas fa-eye"></i> Detail</a></td>
                        </tr>
                        @empty
                        <tr><td colspan="7"><div class="empty-state"><i class="fas fa-gavel"></i><p>Belum ada pengajuan sidang yang perlu ditinjau.</p></div></td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
