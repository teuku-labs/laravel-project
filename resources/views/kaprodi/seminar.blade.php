...
<head>
    ...
    <title>Seminar LKP - Kaprodi</title>
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box;}
        ...
        .badge-status{display:inline-block;padding:.3rem .8rem;border-radius:20px;font-size:.73rem;font-weight:500;}
        .badge-pending{background:rgba(243,156,18,.15);color:#d68910;}
        .badge-done{background:rgba(39,174,96,.15);color:#1e8449;}
...
<aside class="sidebar" id="sidebar">
    <div class="sidebar-header">
        <a href="#" class="sidebar-brand"><i class="fas fa-user-tie"></i><span>Kaprodi Panel</span></a>
    </div>
    <nav class="sidebar-menu">
        <div class="menu-cat">Menu Utama</div>
        <a href="{{ route('kaprodi.dashboard') }}" class="menu-item"><i class="fas fa-home"></i>Dashboard</a>
        <a href="{{ route('kaprodi.seminar') }}" class="menu-item active"><i class="fas fa-clipboard-list"></i>Seminar LKP</a>
...
<div class="main">
    <nav class="topbar">
        <div style="display:flex;align-items:center;gap:1rem;">
            ...
            <span style="color:var(--muted);font-size:.9rem;">Seminar LKP</span>
        </div>
    ...
    <div class="content">
        ...
        <div class="table-card">
            <div class="table-header">
                <div>
                    <strong style="font-size:1rem;">Daftar Seminar LKP</strong>
                    <p style="color:var(--muted);font-size:.83rem;margin:0;">Total: {{ $seminarList->count() }} pendaftar</p>
                </div>
            ...
            <div class="table-responsive">
                <table class="custom-table" id="seminarTable">
                    <thead>
                        <tr>
                            <th>No</th><th>Nama</th><th>NIM</th><th>Judul LKP</th><th>Pembimbing</th><th>Tanggal Daftar</th><th>Status</th><th>Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                @if($s->tanggal_seminar)
                                    <span class="badge-status badge-done"><i class="fas fa-check"></i> Terjadwal: {{ \Carbon\Carbon::parse($s->tanggal_seminar)->format('d M Y') }}</span>
                                @else
                                    <span class="badge-status badge-pending"><i class="fas fa-clock"></i> Menunggu Jadwal</span>
                                @endif
                            </td>
                            <td><a href="{{ route('kaprodi.seminar.show', $s->id) }}" class="btn-detail"><i class="fas fa-eye"></i> Detail</a></td>
                        </tr>
                        @empty
                        <tr><td colspan="8"><div class="empty-state"><i class="fas fa-inbox"></i><p>Belum ada pendaftaran.</p></div></td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
...
<script>
    ...
    document.getElementById('searchInput').addEventListener('input', function() {
        ...
        document.querySelectorAll('#seminarTable tbody tr').forEach(r => {
